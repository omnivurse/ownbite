/*
  # Nutrition Goals and Bloodwork Integration

  1. New Tables
    - `nutrition_goals` - User's daily nutrition targets with profile data
    - `daily_goal_logs` - Track daily progress against goals with streaks
    - `bloodwork_results` - Store uploaded bloodwork files and analysis
    - `nutrient_ranges` - Reference ranges for common nutrients
    - `nutrient_recommendations` - Food recommendations for deficiencies
    - `user_nutrient_status` - Current nutrient status for each user

  2. Functions
    - `calculate_recommended_goals()` - BMR/TDEE-based goal calculation
    - `update_daily_goal_log()` - Auto-update daily progress tracking
    - `analyze_nutrient_status()` - Classify nutrient levels vs. ranges
    - `get_nutrient_recommendations()` - Get personalized food suggestions
...
-- Function to calculate recommended goals
...
BEGIN
  ...
  INSERT INTO daily_goal_logs (
    user_id, log_date, actual_calories, actual_protein_g, actual_fat_g, actual_carbs_g,
    met_calories_goal, met_protein_goal, met_fat_goal, met_carbs_goal, overall_goal_met
  ) VALUES (
    p_user_id, p_date, daily_totals.total_calories, daily_totals.total_protein, 
    daily_totals.total_fat, daily_totals.total_carbs,
...
COMMENT ON SCHEMA public IS 'Added nutrition goals and bloodwork integration with proper conflict handling';