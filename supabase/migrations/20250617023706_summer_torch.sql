/*
  # Bloodwork Trends Tracking System

  1. New Tables
    - `bloodwork_trends` - Stores individual data points for biomarkers over time
      - Enables tracking of specific biomarkers (e.g., iron, vitamin D)
      - Records values with units and dates
      - Links to user accounts

  2. Views
    - `bloodwork_trends_summary` - Aggregates trend data by month for charting
      - Calculates monthly averages for each biomarker
      - Simplifies data retrieval for visualization

  3. Security
    - Enable RLS on bloodwork_trends table
    - Add policies for authenticated users to manage their own data
    - Ensure users can only access their own records

  4. Performance
    - Add index for efficient querying by user and biomarker
*/
...
-- Function to automatically populate bloodwork_trends from user_nutrient_status
...
-- Create trigger to automatically sync trends
